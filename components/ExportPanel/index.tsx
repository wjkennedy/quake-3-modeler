...
interface ExportPanelProps {
  format: 'md3' | 'md5' | 'gltf' | 'pk3' | 'widget';
  onFormatChange: (format: 'md3' | 'md5' | 'gltf' | 'pk3' | 'widget') => void;
  onExport: () => void;
}
...
export function ExportPanel({ format, onFormatChange, onExport }: ExportPanelProps) {
  const handleFormatChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      onFormatChange(e.target.value as 'md3' | 'md5' | 'gltf' | 'pk3' | 'widget');
    },
    [onFormatChange]
  );
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        >
          ...
          <option value="widget">HTML Widget (Embeddable)</option>
        </select>
      </div>
      <button
        onClick={onExport}
      ...
      >
        Download {format === 'widget' ? 'HTML' : format.toUpperCase()}
      </button>
    </div>
  );
}
